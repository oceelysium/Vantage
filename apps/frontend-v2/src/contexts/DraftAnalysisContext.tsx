    ...
    const draftAnalysisConfig = () =>
        isPro()
            ? {
                  ...
                  // Pro: filter on the true pro sample (blended games minus the
                  // prior pseudo-count k), compared directly (no 7-day scaling).
                  minGames: config.proMinGames ?? 20,
                  scaleGamesToWeek: false,
                  gamesOffset: dataset()?.proMeta?.priorGames ?? 0,
              }
            : {
                  ignoreChampionWinrates: config.ignoreChampionWinrates,
                  riskLevel: config.riskLevel,
